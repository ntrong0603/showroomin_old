<?php session_start();
		...
		$objPHPExcel->getActiveSheet()->setCellValue('D1', 'Đánh giá');
		$objPHPExcel->getActiveSheet()->setCellValue('E1', 'Quan tâm');
		$objPHPExcel->getActiveSheet()->setCellValue('F1', 'Lượt đặt hàng');
		$objPHPExcel->getActiveSheet()->setCellValue('G1', 'Tên nhà sản xuất');
		...
		foreach ($thongke_excel as $index => $item) 
		{
			...
			$objPHPExcel->getActiveSheet()->setCellValue('D' . $rowNumber, $item['luotdanhgia']);
			$objPHPExcel->getActiveSheet()->setCellValue('E' . $rowNumber, $item['luotquantam']);
			$objPHPExcel->getActiveSheet()->setCellValue('F' . $rowNumber, $item['luotmua']);
			$objPHPExcel->getActiveSheet()->setCellValue('G' . $rowNumber, $item['ten_nhasx']);
