...
function hienthidiadiem(str){
	...
	*/
	...
	xhttp.open("GET", "templates/place/ajax/ajaxgoiydiadiem.php?diadiem="+str, true);
	xhttp.send(); 
	/*var x = document.getElementById("diadiem").value;
	document.getElementById("textgoiy").innerHTML = x;
	$.ajax({
		type:"get",
		url:"templates/place/ajax/ajaxgoiydiadiem.php",
		data:"diadiem="+x,
		async:false,
		success:function(kq){	
...
<form name="frm" method="POST" action="index.php?com=place&act=save&id=<?php echo @$item['id'];?>&p=<?php echo $_REQUEST['p']?>" enctype="multipart/form-data" class="nhaplieu">	
<div id='tab1' class='' > 
	<?php 
		for($i=1;$i<=$sodmuc;$i++)
		{
	...
	<b>Brand:</b>
		<?php
		$d->reset();
        $sql="select * from table_brand order by stt";
		$d->query($sql);
	    $brand = $d->result_array();
		?>
        <select  name="brand" class="main_font">
			<option value="0">Không thuộc Brand nào</option>
            <?php for($i=0;$i<count($brand);$i++){ ?>
            <option value="<?php echo $brand[$i]['id']?>" <?php if($brand[$i]['id'] == $item['brand']) echo ' selected="selected"'; ?>><?php echo $brand[$i]['ten']?></option>
            <?php }?>	
         </select>
        <br /><br />    
	<b>Concept:</b>
		<?php
		$d->reset();
        $sql="select * from table_concept order by stt";
		$d->query($sql);
	    $concept = $d->result_array();
		?>
        <select  name="concept" class="main_font">
			<option value="0">Không phải Concept</option>
            <?php for($i=0;$i<count($concept);$i++){ ?>
            <option value="<?php echo $concept[$i]['id']?>" <?php if($concept[$i]['id'] == $item['concept']) echo ' selected="selected"'; ?>><?php echo $concept[$i]['ten']?></option>
            <?php }?>	
         </select>
        <br /><br /> 
	<b>Danh mục tỉnh:</b>
	...
	<?php }?>
	<b>Chọn hình:</b> <input type="file" name="file" />  (= hình cùng sản phẩm) <?php echo _place_type?><br />
    <br />
    ...
    <br />
    <b>Tọa độ</b> <input type="text" name="dientich" value="<?php echo @$item['dientich']?>" class="input" /><br /> 
    <b>Link 360</b> <input type="text" name="dientich2" value="<?php echo @$item['dientich2']?>" class="input" /><br /> 
	      <a style="font-size:16px; color:#900;">Thuộc tính </a><br/><br/>
    <div id='' class='' style='width: 90%; height: 200px; overflow:scroll; padding:15px; border:1px solid #ccc;' > 
    ...
    <b>Giá từ</b>
	  <input type="text" name="giacu" value="<?php echo @$item['giacu']?>" class="input" /> &nbsp;<?php echo tien($item['giacu']);?></br>
    <b>đến</b> <input type="text" name="gia" value="<?php echo @$item['gia']?>" class="input" /> &nbsp;<?php echo tien($item['gia']);?></br>
    
    <br />
	<b>Thông tin thêm</b><br/>
	<div>
	 <textarea class="mota" name="thongtinthem" id=""><?php echo @$item['thongtinthem']?></textarea></div> <br/>  
 ...
 <div id='tab3' class='' > 
	...
	<input type="button" value="Thoát" onclick="javascript:window.location='index.php?com=place&act=man'" class="btn" />
</form>
</div>