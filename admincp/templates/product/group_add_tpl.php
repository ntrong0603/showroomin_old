<script language="javascript">		
	...
	{		
		...
		window.location ="index.php?com=place&act=<?php if($_REQUEST['act']=='edit_cat') echo 'edit_cat'; else echo 'add_cat';?><?php if($_REQUEST['id']!='') echo"&id=".$_REQUEST['id']; ?>&id_list_l="+b.value;	
		return true;
	}
	...
	{		
		...
		window.location ="index.php?com=place&act=<?php if($_REQUEST['act']=='edit_cat') echo 'edit_cat'; else echo 'add_cat';?><?php if($_REQUEST['id']!='') echo"&id=".$_REQUEST['id']; ?>&id_list="+b.value+"&id_list_l="+c.value;	
		return true;
	}
...
function get_main_list_l()
	{
		$sql="select * from table_place_list0 order by stt,id asc";
...
function get_main_list()
	{
		...
		if($_REQUEST['id_list_l']!=0)
		{
			...
			$sql="select * from table_place_list where id_list='$id_list_l' order by id asc";
			$stmt=mysql_query($sql);
		}
		else{
...
<form name="frm" method="post" action="index.php?com=place&act=save_cat" enctype="multipart/form-data" class="nhaplieu">	
	...
	<?php }?>
	<b>Hình đại diện:</b> <input type="file" name="file" /> <?php echo _place_type?> - <span style="font-weight:bold; color:red">Kích thước hình: rộng: 210px, cao 170px</span><br />
