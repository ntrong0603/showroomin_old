...
if( isset( $_POST['hoten'] ) ){
	...
	include_once "vr/Vinmus/phpmailer/class.phpmailer.php";
	...
	if(!$mail->Send()) {
		transfer ("Đã có lỗi xảy ra","index.php");
	}
	else{
		nomoney($data);
		transfer ("Đặt hàng thành công","index.php");
	}
	...
	function nomoney($data){
		...
		if( $d->insert($data) ){
				$sql_dathang = "select * from table_dathang order by id desc limit 1 ";
				$d->query($sql_dathang);
				$tam= $d->result_array();
				$dathang=$tam[0]['id'];
