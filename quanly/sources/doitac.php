...
function save_photo(){
	...
	if($id){ // cap nhat
			...
			if($photo = upload_image("file", 'jpg|png|gif|JPG|PNG|GIF', _upload_hinhanh,$file_name)){
				$data['photo'] = $photo;
				$data['thumb'] = create_thumb($data['photo'], 300, 100, _upload_hinhanh,$file_name,2);	
				$d->setTable('doitac');
				$d->setWhere('id', $id);
				$d->select();
				if($d->num_rows()>0){
			...
			for($i=0; $i<4; $i++){
				$file_name=fns_Rand_digit(0,9,6);
				if($photo = upload_image("file".$i, 'jpg|png|gif|JPG|jpeg|JPEG|Jpg|PNG', _upload_hinhanh,$file_name))
					{
						$data['photo'] = $photo;		
						$data['thumb'] = create_thumb($data['photo'], 300, 100, _upload_hinhanh,$file_name,2);
			...
			transfer("Lưu dữ liệu thành công", "index.php?com=doitac&act=man_photo");
