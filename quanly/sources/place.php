...
if(!isset( $_POST['ten'] )){ transfer("Bạn chưa nhập Tên, vui lòng tạo lại", "index.php?com=place&act=man");}
	...
	if($id){
		...
		if($d->insert($data)){
			...
			transfer("Thêm thành công", "index.php?com=place&act=man&id_list=".$abd['id_list']."&id_cat=".$abd['id_cat']."&id_danhmuc=".$abd['id_danhmuc']."&id_item=".$abd['id_item'] );
		}
		else
...
function delete_item(){
	...
	if(isset($_GET['id'])){
		...
		if($d->num_rows()>0){
			...
			$sql_del1 = "delete from #_product where id_place='".$id."'";
			$d->query($sql_del1);
			//xóa hình liên quan
			
			$sql_del2 = "delete from #_hinhsp where com='place' and id_sp='".$id."'";
			$d->query($sql_del2);
			//xóa danh mục
			
			$sql_del3 = "delete from #_product_danhmuc where id_place='".$id."'";
			$d->query($sql_del3);
