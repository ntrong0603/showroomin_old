...
function get_items(){
	...
	$sql = "select * from #_news where loaitin = '".$loaitin."' ";	
...
function get_item(){
	...
	if(!$id)
		transfer("Không nhận được dữ liệu", "index.php?com=news&loaitin=".$loaitin."&act=man");
	
	$sql = "select * from #_news where id='".$id."'";
	$d->query($sql);
	if($d->num_rows()==0) transfer("Dữ liệu không có thực", "index.php?com=news&loaitin=".$loaitin."&act=man");
	$item = $d->fetch_array();	
}

function save_item(){
	global $loaitin,  $d,$item ,$menu,$thuoctinh,$danhmucthuoctinh;
	$file_name=changeTitle($_POST['ten']).time();
	$file_name2=changeTitle($_POST['ten']).time()."-2";
	if(empty($_POST)) transfer("Không nhận được dữ liệu", "index.php?com=news&loaitin=".$loaitin."&act=man");
...
for($i=0;$i<100;$i++){
		...
		if($d->update($data))
			...
			transfer("Cập nhật dữ liệu bị lỗi", "index.php?com=news&loaitin=".$loaitin."&act=man");
	}else{
...
for($i=0;$i<100;$i++){
		...
		$d->setTable('news');
		if($d->insert($data))
			redirect("index.php?com=news&loaitin=".$loaitin."&act=man");
		else
			transfer("Lưu dữ liệu bị lỗi", "index.php?com=news&loaitin=".$loaitin."&act=man");
	}
}
...
function delete_item(){
	...
	if(isset($_GET['id'])){
		...
		if($d->query($sql))
			...
			transfer("Xóa dữ liệu bị lỗi", "index.php?com=news&loaitin=".$loaitin."&act=man");
		...
		} redirect("index.php?com=news&loaitin=".$loaitin."&act=man");} else transfer("Không nhận được dữ liệu", "index.php?com=news&loaitin=".$loaitin."&act=man");
