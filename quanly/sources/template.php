...
function get_item(){
	...
	if(!$id)
		transfer("Không nhận được dữ liệu", "index.php?com=template&act=man");	
	$sql = "select * from #_template where id='".$id."'";
	$d->query($sql);
	if($d->num_rows()==0) transfer("Dữ liệu không có thực", "index.php?com=template&act=man");
	$item = $d->fetch_array();	
}
...
function save_item(){
	...
	if(empty($_POST)) transfer("Không nhận được dữ liệu", "index.php?com=template&act=man");
	...
	if($id){
		...
		if($d->update($data)){
			...
			transfer("Cập nhật dữ liệu bị lỗi", "index.php?com=template&act=man");
	}else{
		if($photo = upload_image("file", 'jpg|png|gif|JPG|jpeg|JPEG', _upload_sanpham,$file_name)){
			$data['photo'] = $photo;		
			$data['thumb'] = create_thumb($data['photo'], 189, 189, _upload_sanpham,$file_name,1);		
		...
		{	
			...
			transfer("Lưu dữ liệu bị lỗi", "index.php?com=template&act=man");
	}
}
...
function delete_item(){
	...
	if(isset($_GET['id'])){
		...
		if($d->query($sql))
			...
			transfer("Xóa dữ liệu bị lỗi", "index.php?com=template&act=man");
	}else transfer("Không nhận được dữ liệu", "index.php?com=template&act=man");
