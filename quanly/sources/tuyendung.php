...
switch($act){
	...
	case "man":
		get_items();
		$template = "tuyendungs/items";
		break;
	case "add":		
		$template = "tuyendungs/item_add";
		break;
	case "edit":		
		get_item();
		$template = "tuyendungs/item_add";
...
function get_items(){
	...
	if($hienthi==0)
	{
	$sqlUPDATE_ORDER = "UPDATE table_tuyendung SET noibat ='$time' WHERE  id = ".$id_up."";
	...
	$url="index.php?com=tuyendungs&act=man".$urlsanpham;
	$maxR=10;
	$maxP=20;
...
function get_item(){
	...
	if(!$id)
		transfer("Không nhận được dữ liệu", "index.php?com=tuyendungs&act=man");	
	$sql = "select * from #_tuyendung where id='".$id."'";
	$d->query($sql);
	if($d->num_rows()==0) transfer("Dữ liệu không có thực", "index.php?com=tuyendungs&act=man");
	$item = $d->fetch_array();	
}
...
function save_item(){
	...
	if(empty($_POST)) transfer("Không nhận được dữ liệu", "index.php?com=tuyendungs&act=man");
	...
	if($id){
		...
		$d->setWhere('id', $id);
		if($d->update($data))
			redirect("index.php?com=tuyendungs&act=man&curPage=".$_REQUEST['curPage']."");
		else
			transfer("Cập nhật dữ liệu bị lỗi", "index.php?com=tuyendungs&act=man");
	}else{
		if($photo = upload_image("file", 'jpg|png|gif|JPG|jpeg|JPEG', _upload_tintuc,$file_name)){
			$data['photo'] = $photo;		
		...
		if($d->insert($data))
		{			
			redirect("index.php?com=tuyendungs&act=man");
		}
		else
			transfer("Lưu dữ liệu bị lỗi", "index.php?com=tuyendungs&act=man");
	}
}
...
function delete_item(){
	...
	if(isset($_GET['id'])){
		$id =  themdau($_GET['id']);
		$d->reset();
		$sql = "select id,thumb, photo from #_tuyendung where id='".$id."'";
		$d->query($sql);
		if($d->num_rows()>0){
		...
		if($d->query($sql))
			redirect("index.php?com=tuyendungs&act=man".$id_catt."");
		else
			transfer("Xóa dữ liệu bị lỗi", "index.php?com=tuyendungs&act=man");
	}else transfer("Không nhận được dữ liệu", "index.php?com=tuyendungs&act=man");
}
