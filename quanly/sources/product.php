...
if(!isset( $_POST['save'] )){ transfer("Không nhận được dữ liệu", "index.php?com=product&act=man&id_place=".$_POST['id_place']);}
	...
	if($id){
		...
		if($photo = upload_image("file", 'jpg|png|gif|JPG|jpeg|JPEG|Jpg|PNG', _upload_sanpham,$file_name)){
			$data['photo'] = $photo;	
			$data['thumb'] = create_thumb($data['photo'], $config['hinhsp']['width'], $config['hinhsp']['height'], _upload_sanpham,$file_name,2);	
	$d->reset();			
		...
		$data['mota2'] = mysql_real_escape_string($_POST['mota2']);
...
function get_danhmucs(){
	...
	$curPage = isset($_GET['p']) ? $_GET['p'] : 1;
	$url="index.php?com=product&act=man_danhmuc&id_place=".$_REQUEST['id_place'];
	$maxR=20;
	$maxP=10;
	$paging=paging($items, $url, $curPage, $maxR, $maxP);
	$items=$paging['source'];
...
function save_danhmuc(){
	...
	if($id){		
		...
		if($photo = upload_image("file", 'jpg|png|gif|JPG|jpeg|JPEG|Jpg|PNG', _upload_sanpham,$file_name)){
			$data['photo'] = $photo;	
			$data['thumb'] = create_thumb($data['photo'], 217, 87, _upload_sanpham,$file_name,2);									
		...
		if($photo = upload_image("file_icon", 'jpg|png|gif|JPG|jpeg|JPEG|Jpg|PNG', _upload_sanpham,$file_name)){
			$data['photo_icon'] = $photo;	
			$data['thumb_icon'] = create_thumb($data['photo_icon'], 217, 87, _upload_sanpham,$file_name,2);									
			$d->reset();$d->setTable('product_danhmuc');
			$d->setWhere('id', $id);
			$d->select();
			if($d->num_rows()>0){
				$row = $d->fetch_array();
				delete_file(_upload_sanpham.$row['photo_icon']);	
				delete_file(_upload_sanpham.$row['thumb_icon']);	
