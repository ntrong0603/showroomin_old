...
switch($act){
	case "man_photo":
		get_photos();
		$template = "xuatkho/photos";
		break;
	case "add_photo":
		$template = "xuatkho/photo_add";
		break;
	case "edit_photo":
		$_SESSION['xuatkho']="index.php?com=lichtrinh&act=edit_photo&id=".$_GET['id']."&id_sp=".$_GET['id_sp'];
		get_photo();
		$template = "xuatkho/photo_add";
		break;
	case "save_photo":
...
function get_photos(){
	...
	$sql_sp = "SELECT id,hienthi FROM table_xuatkho where id='".$id_up."' ";
	$d->query($sql_sp);
	$cats= $d->result_array();
	$hienthi=$cats[0]['hienthi'];
	if($hienthi==0)
	{
	$sqlUPDATE_ORDER = "UPDATE table_xuatkho SET hienthi =1 WHERE  id = ".$id_up."";
	$resultUPDATE_ORDER = mysql_query($sqlUPDATE_ORDER) or die("Not query sqlUPDATE_ORDER");
	}
	else
	{
	$sqlUPDATE_ORDER = "UPDATE table_xuatkho SET hienthi =0  WHERE  id = ".$id_up."";
	$resultUPDATE_ORDER = mysql_query($sqlUPDATE_ORDER) or die("Not query sqlUPDATE_ORDER");
	}	
	}
	#-------------------------------------------------------------------------------
...
function save_photo(){
	...
	if($id){ // cap nhat
			...
			$d->setTable('xuatkho');
			$d->setWhere('id', $id);
			if(!$d->update($data)) transfer("Cập nhật dữ liệu bị lỗi", "index.php?com=lichtrinh&act=man_photo");
