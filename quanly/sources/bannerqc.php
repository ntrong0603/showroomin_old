...
function save_banner(){
	...
	$file_name1=fns_Rand_digit(0,9,5);
	$sql = "select * from #_photo where com='banner_top'";
	$d->query($sql);
	$item = $d->fetch_array();
	$id=$item['id'];
	...
	if($id){ // cap nhat
		...
		if($photo1 = upload_image("file1", 'swf|jpg|png|gif', _upload_hinhanh,$file_name1)){
			$data['photo_sd'] = $photo1;
			$d->setTable('photo');
			$d->setWhere('id', $id);
			$d->select();
			$row = $d->fetch_array();
			delete_file(_upload_hinhanh.$row['photo_sd']);
		}
...
function save_banner_giua(){
	...
	if($id){ // cap nhat
		...
		$data['photo'] = upload_image("file", 'swf|jpg|png|gif', _upload_hinhanh,$file_name);
		if(!$data['photo']) $data['photo']="";
