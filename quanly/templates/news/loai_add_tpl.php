...
<script language="javascript">				
	...
	{				
		...
		window.location ="index.php?com=place&act=<?php if($_REQUEST['act']=='edit_item') echo 'edit_item'; else echo 'add_item';?><?php if( isset($_REQUEST['id']) and $_REQUEST['id']!='') echo"&id=".$_REQUEST['id']; ?>&id_danhmuc="+b.value;	
		return true;
	}
	...
	{				
		...
		window.location ="index.php?com=place&act=<?php if($_REQUEST['act']=='edit_item') echo 'edit_item'; else echo 'add_item';?><?php if( isset($_REQUEST['id']) and $_REQUEST['id']!='') echo"&id=".$_REQUEST['id']; ?>&id_danhmuc="+a.value+"&id_list="+b.value;	
		return true;
	}
...
<?php	
	...
	{
		global $loaitin;
		$sql_huyen="select * from table_place_cat where loaitin='".$loaitin."'  and id_list=".$_REQUEST['id_list']." order by stt,id desc ";
	...
	{
		global $loaitin;
		$sql_huyen="select * from table_place_list  where loaitin ='".$loaitin."' and id_danhmuc=".$_REQUEST['id_danhmuc']."  order by stt,id desc ";
		$result=mysql_query($sql_huyen);
		$str='
	...
	{
		global $loaitin;
		$sql_huyen="select * from table_place_danhmuc where loaitin ='".$loaitin."' order by stt,id desc ";
		$result=mysql_query($sql_huyen);
		$str='
...
<form name="frm" method="post" action="index.php?com=place&act=save_item&id=<?php echo @$item['id']?>" enctype="multipart/form-data" class="nhaplieu">	
	...
	<?php }?>
	<b>Chọn hình đại diện:</b> <input type="file" name="file" /> <?php echo _place_type?><br />
    <br />
	...
	<input type="button" value="Thoát" onclick="javascript:window.location='index.php?com=place&act=man_item'" class="btn" />
</form>