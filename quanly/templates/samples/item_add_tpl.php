...
<?php if ($_REQUEST['act']=='edit') { ?> <h3>Sửa samples</h3> <?php }else{ ?><h3>Thêm samples</h3><?php } ?>
<script language="javascript">				
	function select_onchange()
	{				
		var a=document.getElementById("id_danhmuc");
		window.location ="index.php?com=samples&act=<?php if($_REQUEST['act']=='edit') echo 'edit'; else echo 'add';?><?php if( isset($_REQUEST['id']) and $_REQUEST['id']!='') echo"&id=".$_REQUEST['id']; ?>&id_danhmuc="+a.value;	
		return true;
	}
	...
	{				
		...
		window.location ="index.php?com=samples&act=<?php if($_REQUEST['act']=='edit') echo 'edit'; else echo 'add';?><?php if( isset($_REQUEST['id']) and $_REQUEST['id']!='') echo"&id=".$_REQUEST['id']; ?>&id_danhmuc="+a.value+"&id_list="+b.value;	
		return true;
	}
	...
	{				
		...
		window.location ="index.php?com=samples&act=<?php if($_REQUEST['act']=='edit') echo 'edit'; else echo 'add';?><?php if( isset($_REQUEST['id']) and $_REQUEST['id']!='') echo"&id=".$_REQUEST['id']; ?>&id_danhmuc="+a.value+"&id_list="+b.value+"&id_cat="+c.value;	
		return true;
	}
...
?>
<form name="frm" method="post" action="index.php?com=samples&act=save&p=<?php echo $_REQUEST['p']?>" enctype="multipart/form-data" class="nhaplieu">	 
	
    <?php if ($_REQUEST['act']==edit)
	{?>
	<b>Hình ảnh:</b><img src="<?php echo _upload_sanpham.$item['thumb']?>"  alt="NO PHOTO" width="200px;" /><br />
	<?php }?>
	<b>Chọn hình:</b> <input type="file" name="file" /> <?php echo _samples_type?><br />
    <br />
    ...
    <b>Mã sản phẩm</b> <input type="text" name="maso" value="<?php echo @$item['maso']?>" class="input" /><br />    
    <b>Chất liệu</b> <input type="text" name="chatlieu" value="<?php echo @$item['chatlieu']?>" class="input" /><br />    
    ...
    <b>Featured samples </b> <input type="checkbox" name="noibat" <?php echo ( $item['noibat']==1)?'checked="checked"':''?> /><br /> 
	...
	<input type="button" value="Thoát" onclick="javascript:window.location='index.php?com=samples&act=man'" class="btn" />
</form>