	...
	<?php if ($capmenu>=2){ ?>	<td style="width:10%;">
      ...
      <?php if ($capmenu>=4){ ?>  <td style="width:10%;">
       ...
       <?php  } ?>
		...
		?>
  		<a href="index.php?com=hinhsp&act=man_photo&id_place=<?=@$items[$i]['id_place']?>&id_sp=<?=$items[$i]['id']?>" style="text-decoration:none;"><img src="media/images/Photos-icon.png" alt="gallery" width="30" ><span><br /><?=count($result_g);?> images</span></a>
        
      </td>
        
        
        <td style="width:5%;">
        	<a href="index.php?com=product&act=man&id_place=<?=@$items[$i]['id_place']?>&spmoi=<?=@$items[$i]['id']?>"><img src="media/images/active_<?=(@$items[$i]['spmoi'] > 0)?1:0?>.png" border="0" /></a> 
        </td>
  
        <td style="width:5%;">
        	<a href="index.php?com=product&act=man&id_place=<?=@$items[$i]['id_place']?>&khuyenmai=<?=@$items[$i]['id']?>"><img src="media/images/active_<?=(@$items[$i]['khuyenmai'] > 0)?1:0?>.png" border="0" /></a> 
        </td>
        
          
		<td style="width:5%;">
        	<a href="index.php?com=product&act=man&id_place=<?=@$items[$i]['id_place']?>&spbanchay=<?=@$items[$i]['id']?>"><img src="media/images/active_<?=(@$items[$i]['spbanchay'] > 0)?1:0?>.png" border="0" /></a> 
        </td>
        
        
        <td style="width:5%;">
        	<a href="index.php?com=product&act=man&id_place=<?=@$items[$i]['id_place']?>&hienthi=<?=@$items[$i]['id']?>"><img src="media/images/active_<?=(@$items[$i]['hienthi'] > 0)?1:0?>.png" border="0" /></a> 
        </td>
        
		<td style="width:5%;"><a href="index.php?com=product&act=edit&id_place=<?=@$items[$i]['id_place']?>&id_danhmuc=<?=@$items[$i]['id_danhmuc']?>&id_list=<?=@$items[$i]['id_list']?>&id_cat=<?=@$items[$i]['id_cat']?>&id_item=<?=@$items[$i]['id_item']?>&id=<?=@$items[$i]['id']?>"><img src="media/images/edit.png" /></a></td>
		<td style="width:5%;"><a href="index.php?com=product&act=delete&id_place=<?=@$items[$i]['id_place']?>&id=<?=@$items[$i]['id']?>" onClick="if(!confirm('Xác nhận xóa: <?=@$items[$i]['ten']?>')) return false;"><img src="media/images/delete.png" /></a></td>
	</tr>
	<?php	}?>
    </table>
<a href="index.php?com=product&act=add&id_place=<?php echo $_GET['id_place']?>"><img src="media/images/add.jpg" border="0"  /></a>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<a href="#" id="xoahet"><img src="media/images/delete.jpg" border="0"  /></a>
