...
$("#xoahet").click(function(){
	...
	if (hoi==true) document.location = "index.php?com=brand&act=delete_cat&listid=" + listid;
});
});
</script>
...
<script language="javascript">				
	...
	{				
		...
		window.location ="index.php?com=brand&act=man_cat&id_danhmuc="+a.value;	
		return true;
	}
	...
	{				
		...
		window.location ="index.php?com=brand&act=man_cat&id_danhmuc="+a.value+"&id_list="+b.value;	
		return true;
	}
...
function get_main_list()
	{
		$sql="select * from table_brand_list where id_danhmuc=".$_REQUEST['id_danhmuc']."  order by stt";
		$stmt=mysql_query($sql);
		$str='
...
<table class="blue_table">
    ...
    <tr bgcolor="<?php if($i%2==1)echo '#F3F3F3' ?>">
        ...
        <td align="center" style="width:20%;">       
			<?php
            $sql_danhmuc="select ten from table_brand_list where id='".$items[$i]['id_list']."'";
            $result=mysql_query($sql_danhmuc);
            $item_danhmuc =mysql_fetch_array($result);
            echo @$item_danhmuc['ten']
            ?>         
		...
		<td style="width:5%;"><a href="index.php?com=brand&act=edit_cat&id_danhmuc=<?php echo @$items[$i]['id_danhmuc']?>&id_list=<?php echo @$items[$i]['id_list']?>&id=<?php echo @$items[$i]['id']?>"><img src="media/images/edit.png" border="0" /></a></td>
		<td style="width:5%;"><a href="index.php?com=brand&act=delete_cat&id=<?php echo @$items[$i]['id']?>" onClick="if(!confirm('Xác nhận xóa: <?php echo @$items[$i]['ten']?>')) return false;"><img src="media/images/delete.png" border="0" /></a></td>
	</tr>
	<?php	}?>
</table>
