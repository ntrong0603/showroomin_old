...
function hienthidiadiem(str){
	...
	*/
	...
	xhttp.open("GET", "templates/chothue/ajax/ajaxgoiydiadiem.php?diadiem="+str, true);
	xhttp.send(); 
	/*var x = document.getElementById("diadiem").value;
	document.getElementById("textgoiy").innerHTML = x;
	$.ajax({
		type:"get",
		url:"templates/chothue/ajax/ajaxgoiydiadiem.php",
		data:"diadiem="+x,
		async:false,
		success:function(kq){	
...
<form name="frm" method="POST" action="index.php?com=chothue&act=save&id=<?php echo @$item['id'];?>&p=<?php echo $_REQUEST['p']?>" enctype="multipart/form-data" class="nhaplieu">	
<div id='tab1' class='' > 
	<?php 
		for($i=1;$i<=$sodmuc;$i++)
		{
	...
	<b>Danh mục tỉnh:</b><span style='color:red'>code bắt buộc chọn mục này</span>
        ...
        <b>Danh mục huyện:</b>
        <div id="chonhuyen">
            <select id="id_huyen" name="id_huyen" class="main_font" onchange="load_phuong(),load_chothue()">
	...
	{?>
	<b>Hình ảnh:</b><img src="<?php echo "../upload/chothue/".$item['thumb']?>"  alt="NO PHOTO" width="200px;" /><br />
	<?php }?>
	<b>Chọn hình:</b> <input type="file" name="file" />  (= hình cùng sản phẩm) <?php echo _chothue_type?><br />
    <br />
  ...
  <b>Địa chỉ</b> 
  <input type="text" id="diadiem" name="diadiem" onclick="hienthi()" onkeyup="hienthidiadiem(this.value)" value="<?php echo @$item['diadiem']?>" class="input" />
    	
	
    <br />
    <b>Diện tích</b> <input type="text" name="dientich" value="<?php echo @$item['dientich']?>" class="input" />   m2 <br /> 
    <b>Thời hạn</b> <input type="text" name="thoihan" value="<?php echo @$item['thoihan']?>" class="input" />   Tháng <br /> 
    <b>Đặt cọc</b> <input type="text" name="datcoc" value="<?php echo @$item['datcoc']?>" class="input" />   Tháng <br /> 
    
    <b>Giá </b> <input type="text" name="gia" value="<?php echo @$item['gia']?>" class="input" /> &nbsp;<b><< <?php echo chuyentien($item['gia']);?> >></b>
	<input type="radio" name="giacu" value="0" <?php if( isset( $item ) and $item['giacu']==1 ){;}else{ echo "checked"; }?> /> /tháng  <input type="radio" name="giacu" value="1"   <?php if( isset( $item ) and $item['giacu']==1 ){ echo "checked"; }?> /> /m2
 ...
 <script language="javascript">CKEDITOR.replace('congtrinh_rd');</script> 
	...
	<input type="button" value="Thoát" onclick="javascript:window.location='index.php?com=chothue&act=man'" class="btn" />
</form>
</div>