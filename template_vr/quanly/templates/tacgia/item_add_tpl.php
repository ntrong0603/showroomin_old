...
<?php if ($_REQUEST['act']=='edit') { ?> <h3>Sửa tác giả</h3> <?php }else{ ?><h3>Thêm tác giả</h3><?php } ?>
<?php
function get_main_item()
	{
		$sql_huyen="select * from table_tacgia_item order by stt,id desc ";
...
?>
<form name="frm" method="post" action="index.php?com=tacgia&act=save" enctype="multipart/form-data" class="nhaplieu">
	...
	<b>Tên tác giả</b> <input type="text" name="ten" value="<?php echo @$item['ten']?>" class="input" /><br /><br />
    <b>Website cá nhân tác giả</b> <input type="text" name="mota" value="<?php echo @$item['mota']?>" class="input" /><br /><br />
	...
	<input type="button" value="Thoát" onclick="javascript:window.location='index.php?com=tacgia&act=man'" class="btn" />
</form>