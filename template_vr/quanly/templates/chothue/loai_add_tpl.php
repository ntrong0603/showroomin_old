...
<script language="javascript">				
	...
	{				
		...
		window.location ="index.php?com=chothue&act=<?php if($_REQUEST['act']=='edit_item') echo 'edit_item'; else echo 'add_item';?><?php if( isset($_REQUEST['id']) and $_REQUEST['id']!='') echo"&id=".$_REQUEST['id']; ?>&id_danhmuc="+b.value;	
		return true;
	}
	...
	{				
		...
		window.location ="index.php?com=chothue&act=<?php if($_REQUEST['act']=='edit_item') echo 'edit_item'; else echo 'add_item';?><?php if( isset($_REQUEST['id']) and $_REQUEST['id']!='') echo"&id=".$_REQUEST['id']; ?>&id_danhmuc="+a.value+"&id_list="+b.value;	
		return true;
	}
...
<?php	
	...
	{
		$sql_huyen="select * from table_chothue_cat where id_list=".$_REQUEST['id_list']." order by stt,id desc ";
	...
	{
		$sql_huyen="select * from table_chothue_list where id_danhmuc=".$_REQUEST['id_danhmuc']."  order by stt,id desc ";
		$result=mysql_query($sql_huyen);
		$str='
	...
	{
		$sql_huyen="select * from table_chothue_danhmuc order by stt,id desc ";
		$result=mysql_query($sql_huyen);
		$str='
...
<form name="frm" method="post" action="index.php?com=chothue&act=save_item&id=<?php echo @$item['id'];?>" enctype="multipart/form-data" class="nhaplieu">	
	...
	{?>
	<b>Hình ảnh đại diện:</b><img src="<?php echo "../upload/chothue".$item['thumb']?>"  alt="NO PHOTO" width="200px;" /><br />
	<?php }?>
	<b>Chọn hình đại diện:</b> <input type="file" name="file" /> <?php echo _chothue_type?><br />
    <br />
	...
	<input type="button" value="Thoát" onclick="javascript:window.location='index.php?com=chothue&act=man_item'" class="btn" />
</form>