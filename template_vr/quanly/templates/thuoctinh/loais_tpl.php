...
function get_main_danhmuc()
	{
		global $loaitin;
		$sql="select * from table_product_danhmuc where loaitin ='".$loaitin."' order by stt";
...
function get_main_list()
	{
		global $loaitin;
		$sql="select * from table_product_list  where loaitin ='".$loaitin."' and id_danhmuc=".$_REQUEST['id_danhmuc']."  order by stt";
		$stmt=mysql_query($sql);
		$str='
...
function get_main_category()
	{
		global $loaitin;
		$sql="select * from table_product_cat where loaitin='".$loaitin."' and 'id_list=".$_REQUEST['id_list']." order by stt";
		$stmt=mysql_query($sql);
		$str='
...
<table class="blue_table">
	...
	?>
        ...
        <?php
		$sql_danhmuc="select ten from table_product_list  where loaitin ='".$loaitin."' and id='".$items[$i]['id_list']."'";
		$result=mysql_query($sql_danhmuc);
	 	$item_danhmuc =mysql_fetch_array($result);
	 	echo @$item_danhmuc['ten']
	?>
	...
	?>
		...
		<td style="width:30%;"><a href="index.php?com=product&act=edit_item&id_list=<?=@$items[$i]['id_list']?>&id_cat=<?=@$items[$i]['id_cat']?>&id=<?=@$items[$i]['id']?>&p=<?=$_REQUEST['p']?>" style="text-decoration:none;"><?=@$items[$i]['ten']?> </a></td>
		<td style="width:5%;">
		<?php 
