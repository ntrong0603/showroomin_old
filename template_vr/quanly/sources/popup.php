...
function get_items(){
	...
	$url="index.php?com=popup&act=man&tinhtrang=".$_GET['tinhtrang'];
	$maxR=20;
	$maxP=20;
...
function get_item(){
	...
	$d->query($sql);
	if($d->num_rows()==0) transfer("Dữ liệu không có thực", "index.php?com=popup&act=man");
	$item = $d->fetch_array();
	if($item['xem'] == 0){
		$data['xem'] = 1;		
		$d->setTable('popup');
...
function save_item(){
	...
	if(empty($_POST)) transfer("Không nhận được dữ liệu", "index.php?com=popup&act=man");
	...
	if($id){
		...
		$d->setWhere('id', $id);
		if($d->update($data))
			redirect("index.php?com=popup&act=man&curPage=".$_REQUEST['curPage']."");
		else
...
function delete_item(){
	...
	if(isset($_GET['id']))
	{
		...
		$d->query($sql);
		if($d->query($sql))
			redirect("index.php?com=popup&act=man".$id_catt."");
		else
	...
	else transfer("Không nhận được dữ liệu", "index.php?com=popup&act=man");
}
