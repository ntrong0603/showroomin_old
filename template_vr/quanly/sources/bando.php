...
function get_item(){
	...
	if(!$id)
		transfer("Không nhận được dữ liệu", "index.php?com=bando&act=man");	
	$sql = "select * from #_map where id='".$id."'";
	$d->query($sql);
	if($d->num_rows()==0) transfer("Dữ liệu không có thực", "index.php?com=bando&act=man");
	$item = $d->fetch_array();	
}
...
function save_item(){
	...
	if(empty($_POST)) transfer("Không nhận được dữ liệu", "index.php?com=bando&act=man");
	...
	if($id){
		...
		if($d->update($data))
			...
			transfer("Cập nhật dữ liệu bị lỗi", "index.php?com=bando&act=man");
	}else{
		$data['toado'] = mysql_real_escape_string($_POST['toado']) ;
		$data['address'] = mysql_real_escape_string($_POST['address']) ;
		...
		{			
			...
			transfer("Lưu dữ liệu bị lỗi", "index.php?com=bando&act=man");
	}
}
...
function delete_item(){
	...
	if(isset($_GET['id'])){
		...
		if($d->query($sql))
			...
			transfer("Xóa dữ liệu bị lỗi", "index.php?com=bando&act=man");
	}else transfer("Không nhận được dữ liệu", "index.php?com=bando&act=man");
}
