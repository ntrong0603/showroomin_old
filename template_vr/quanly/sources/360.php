...
function get_item(){
	...
	if(!$id)
		transfer("Không nhận được dữ liệu", "index.php?com=360&act=man");
...
function save_item(){
	...
	if(empty($_POST)) transfer("Không nhận được dữ liệu", "index.php?com=360&act=man");
	...
	if($id){
		...
		if($d->update($data))
			...
			transfer("Cập nhật dữ liệu bị lỗi", "index.php?com=360&act=man");
	}else{
		...
		if($d->insert($data))
			...
			transfer("Lưu dữ liệu bị lỗi", "index.php?com=360&act=man");
	}
}
...
function delete_item(){
	...
	if(isset($_GET['id'])){
		...
		if($d->query($sql))
			...
			transfer("Xóa dữ liệu bị lỗi", "index.php?com=360&act=man");
		...
		} redirect("index.php?com=360&act=man");} else transfer("Không nhận được dữ liệu", "index.php?com=360&act=man");
...
function get_danhmuc(){
	...
	$id = isset($_GET['id']) ? themdau($_GET['id']) : "";
	if(!$id)
	transfer("Không nhận được dữ liệu", "index.php?com=360&act=man_danhmuc");
	
	$sql = "select * from #_360 where id='".$id."'";
	$d->query($sql);
	if($d->num_rows()==0) transfer("Dữ liệu không có thực", "index.php?com=360&act=man_danhmuc");
	$item = $d->fetch_array();	
}
...
function save_danhmuc(){
	global $d;
	$file_name=fns_Rand_digit(0,9,6);
	if(empty($_POST)) transfer("Không nhận được dữ liệu", "index.php?com=360&act=man_danhmuc");
	$id = isset($_POST['id']) ? themdau($_POST['id']) : "";
	if($id){		
		$id =  themdau($_POST['id']);
		...
		if($d->update($data))
			...
			transfer("Cập nhật dữ liệu bị lỗi", "index.php?com=360&act=man_danhmuc");
	}else{			
		...
		if($d->insert($data))
			...
			transfer("Lưu dữ liệu bị lỗi", "index.php?com=360&act=man_danhmuc");
	}
}
...
function delete_danhmuc(){
	...
	if(isset($_GET['id'])){
		...
		if($d->query($sql))
			...
			transfer("Xóa dữ liệu bị lỗi", "index.php?com=360&act=man_danhmuc");
	}
	elseif (isset($_GET['listid'])==true)
	{
		$listid = explode(",",$_GET['listid']); 
		...
		} redirect("index.php?com=360&act=man_danhmuc");} else transfer("Không nhận được dữ liệu", "index.php?com=360&act=man_danhmuc"	    );
}
?>
