<?php 
	...
	if(count($result_cart)>0){
		...
		for($i_gioh=0; $i_gioh<count($result_cart); $i_gioh++){
			...
			$result_gioh.="<div class='col-xs-3' style='padding: 0px;'>";
			$result_gioh.="<img src='./../../"._upload_sanpham_l.$item['photo']."' width='100%' alt='sản phẩm' title='sản phẩm'>";
			$result_gioh.="</div>";
			$result_gioh.="<div class='col-xs-9' style='padding: 0px;padding-left: 13px;    font-weight: bold;'>";
			$result_gioh.="<div class='col-xs-6' style='font-size: 12px;color: #252525;    padding: 0px 2px;'>".$item['ten']."</div>";
			...
			$result_gioh.="<div class='col-xs-5' style='font-size: 12px;color: #ff0000;font-weight: bold;padding: 0px 2px; text-align:right;'>".number_format($new_price,0,'','.')."";
			if($item['khuyenmai']>0){
			$result_gioh.="<div>";
		...
		$result_gioh.="<div style='margin: 10px 0px;font-size: 13px;color:#2d2d2d;font-weight: bold;'>Thành tiền: <span style='color: #ff0000;font-weight: bold; float: right;'>".number_format($tonggia,0,'','.')." VNĐ</span> <br/> <span style='font-size: 11px;float: right;color:#2d2d2d;font-style:italic; font-weight:500;'>Đã bao gồm thuế VAT</span></div>";
